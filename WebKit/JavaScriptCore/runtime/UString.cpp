...
/* FYWEBKITMOD BEGIN */
#if PLATFORM(FYMP)
	// original code could not handle negative hex numbers (we add a hacky fix)
    if ((data[0] == '0' && data + 2 < end && (data[1] | 0x20) == 'x' && isASCIIHexDigit(data[2]))
	  ||(data[0] == '-' && data + 3 < end && data[1] == '0' && (data[2] | 0x20) == 'x' && isASCIIHexDigit(data[3]))) {
    	bool bSigned = (data[0] == '-');
        // Hex number.
        data += bSigned ? 3 : 2;
#else
