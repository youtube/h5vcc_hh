...
/* FYWEBKITMOD BEGIN */
#elif PLATFORM(FYMP)
    void *address = FYMPwebkitGlue::RTmalloc(BLOCK_SIZE, BLOCK_SIZE);
/* FYWEBKITMOD END */
...
/* FYWEBKITMOD BEGIN implemented 'FIXED_FREESIZE_GC' */
#ifdef ENABLE_FIXED_FREESIZE_GC
    size_t minCellCount = usedCellCount + ALLOCATIONS_PER_COLLECTION;
    size_t maxCellCount = minCellCount + (ALLOCATIONS_PER_COLLECTION * FIXED_FREESIZE_FACTOR);
#else
    size_t minCellCount = usedCellCount + max(ALLOCATIONS_PER_COLLECTION, usedCellCount);
    size_t maxCellCount = minCellCount + 1.25f * minCellCount;
#endif
/* FYWEBKITMOD END */
...
/* FYWEBKITMOD BEGIN */
#elif PLATFORM(FYMP)
    return FYMPwebkitGlue::currentThreadStackBase();
/* FYWEBKITMOD END */
#elif OS(UNIX)
    AtomicallyInitializedStatic(Mutex&, mutex = *new Mutex);
    MutexLocker locker(mutex);
