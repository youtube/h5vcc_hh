...
namespace JSC {
    ...
    class FYMP_PRXSYM_WEBKIT JS_EXPORTCLASS JSString : public JSCell { // FYWEBKITMOD exported symbol
    public:
        friend class JIT;
    ...
    inline UString JSValue::toString(ExecState* exec) const
    {
        ...
        if (isInt32())
            return exec->globalData().numericStrings.add((int)asInt32()); // FYWEBKITMOD typecast to int
        if (isDouble())
