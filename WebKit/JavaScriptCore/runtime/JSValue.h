...
namespace JSC {
    ...
    inline JSValue::JSValue(ExecState* exec, double d)
    {
        ...
        if (asInt32 != d
            || (!asInt32 && signbit(d)) // true for -0.0
/* FYWEBKITMOD BEGIN */
#if PLATFORM(FYMP)
            || (asInt32==0x80000000)    // we don't want to use max signed negative integer,
#endif /* FYWEBKITMOD END */          //    because it negates into the same value instead
			)                           //    of a positive one
        {
            u.asDouble = d;
...
/* FYWEBKITMOD BEGIN */
#if PLATFORM(FYMP)
            if ( isnan(-d) ) u.asDouble = nonInlineNaN();
#endif
/* FYWEBKITMOD END */
            return;
        }
...
/* FYWEBKITMOD END */
    ...
    {
/* FYWEBKITMOD BEGIN check on overflow instead comparison */
        if (static_cast<int32_t>(i) < 0) {
            *this = JSValue(exec, static_cast<double>(i));
            return;
        }
/* FYWEBKITMOD END */
    ...
    {
        const int32_t asInt32 = static_cast<int>(d); // FYWEBKITMOD typecast to int
        if (asInt32 != d || (!asInt32 && signbit(d))) { // true for -0.0
            u.asDouble = d;
            return;
        }
        *this = JSValue(globalData, static_cast<int>(d)); // FYWEBKITMOD typecast to int
    }
    ...
    {
        if (static_cast<int>(i) < 0) { // FYWEBKITMOD typecast to int
            *this = JSValue(globalData, static_cast<double>(i));
            return;
        }
        *this = JSValue(globalData, static_cast<int>(i)); // FYWEBKITMOD typecast to int
    }
