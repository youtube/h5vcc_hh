...
/* FYWEBKITMOD BEGIN */
    ...
    /*
        GHS compiler oddity fix:

        The GHS compiler puts the vtbl pointer at the END of the BASE class of any
        class, not at offset zero.

        With multiple inherintance it's not trivial to speak of "THE" base class.

        When the using "THE" base class to abstrcatly handle >1 types with a central
        piece of code, things can go nastily wrong! Here the virtual destructor
        is called on ParserArenaDeletable; expecting the delete CommaNode correctly.
        It goes wrong because GHS put the ExpressionNode data FIRST, not ParserArenaDeletable,
        while the code deleting it expected otherwise (only knowing ParserArenaDeletable).

        YIKES!

        Changing the order(!) of the inhereted classes here fixes this!
    */
