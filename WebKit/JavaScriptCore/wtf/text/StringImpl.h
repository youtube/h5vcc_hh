...
class FYMP_PRXSYM_WEBKIT StringImpl : public StringImplBase { // FYWEBKITMOD exported symbol
...
public:
	...
	static PassRefPtr<StringImpl> reallocate(PassRefPtr<StringImpl> originalString, unsigned length, UChar*& data); // FYMP - integrating parts of r152754
...
// There are also assembler versions for the two equal()'s below conditional to set #if CPU(...).
// I dont know if that macro works for us, so I decided for these versions which I understand.
