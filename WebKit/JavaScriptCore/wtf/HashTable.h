...
#if defined(NDEBUG) || (PLATFORM(FYMP) && (FYMP_PLATFORM == FYMP_PLATFORM_CTR || FYMP_PLATFORM == FYMP_PLATFORM_VITA)) // FYWEBKITMOD including some platforms where we can't have this debug check here as too few mutex instances are available!
...
#define CHECK_HASHTABLE_USE_AFTER_DESTRUCTION 1
...
#if !COMPILER(MSVC) && !COMPILER(MWERKS) && !COMPILER(CLANG) && !(defined(FYMP_PLATFORM) && FYMP_PLATFORM == FYMP_PLATFORM_PS3) && !COMPILER(GHS) && !(defined(FYMP_PLATFORM) && FYMP_PLATFORM == FYMP_PLATFORM_VITA) // FYWEBKITMOD excluding platforms with a swap for pairs defined
    ...
    private:
        ...
        ALWAYS_INLINE iterator makeIterator(ValueType* pos) { return iterator(this, pos, m_table + m_tableSize); }			// FYMP: forcing inline compilation
