...
#ifndef SegmentedString2_h
#define SegmentedString2_h
...
/* FYWEBKITMOD
This file was integrated from r152754. It was decided that we keep old WTFString. 
This old string class always is utf-16. Thus, some changes are applied in this file. 
*/

class SegmentedString2;

class SegmentedSubstring2 {
public:
    SegmentedSubstring2()
        : m_length(0)
    ...
    SegmentedSubstring2(const String& str)
        : m_length(str.length())
        , m_doNotExcludeLineNumbers(true)
        , m_string(str)
    {
/* FYWEBKITMOD BEGIN - keeping old WTFString
        if (m_length) {
...
*/
		if (m_length)
			{
			m_data.string16Ptr = m_string.characters();
			}
/* FYWEBKITMOD END */
    }
    ...
    bool is8Bit() { return false; /*m_is8Bit;*/ } // FYWEBKITMOD - keeping old WTFString
    ...
    void appendTo(WTF::StringBuilder2& builder) const
    {
...
/* FYWEBKITMOD BEGIN - integrating parts of r152754 but keeping old WTFString being always 16bit.
...
FYWEBKITMOD END */
    ...
    {
        ASSERT(m_length);
/* FYWEBKITMOD BEGIN - keeping old 16bit WTFString
        if (is8Bit())
...
public:
/* FYWEBKITMOD BEGIN - keeping old WTFString which is always 16bit
	union {
...
public:
    ...
    SegmentedString2(const String& str)
        : m_pushedChar1(0)
        , m_pushedChar2(0)
        , m_currentString(str)
        , m_currentChar(0)
    ...
    SegmentedString2(const SegmentedString2&);

    const SegmentedString2& operator=(const SegmentedString2&);

    void clear();
    void close();

    void append(const SegmentedString2&);
    void prepend(const SegmentedString2&);
...
// FYWEBKITMOD BEGIN using old 16bit WTFString behind the scenes
		ASSERT(m_fastPathFlags & (Use8BitAdvance == 0));
...
private:
    ...
    void append(const SegmentedSubstring2&);
    void prepend(const SegmentedSubstring2&);

//    void advance8(); FYWEBKITMOD - using old 16bit WTFString behind the scenes.
    void advance16();
//    void advanceAndUpdateLineNumber8(); FYWEBKITMOD - using old 16bit WTFString behind the scenes.
...
/* FYWEBKITMOD BEGIN - keeping old WTFString which is always 16bit
			if (m_currentString.is8Bit()) {
                m_advanceFunc = &SegmentedString2::advance8;
                m_fastPathFlags = Use8BitAdvance;
                if (m_currentString.doNotExcludeLineNumbers()) {
                    m_advanceAndUpdateLineNumberFunc = &SegmentedString2::advanceAndUpdateLineNumber8;
                    m_fastPathFlags |= Use8BitAdvanceAndUpdateLineNumbers;
                } else
                    m_advanceAndUpdateLineNumberFunc = &SegmentedString2::advance8;
                return;
            }
FYWEBKITMOD END */
            m_advanceFunc = &SegmentedString2::advance16;
            m_fastPathFlags = NoFastPath;
            if (m_currentString.doNotExcludeLineNumbers())
                m_advanceAndUpdateLineNumberFunc = &SegmentedString2::advanceAndUpdateLineNumber16;
            else
                m_advanceAndUpdateLineNumberFunc = &SegmentedString2::advance16;
            return;
        }
    ...
    LookAheadResult lookAheadSlowCase(const String& string, bool caseSensitive)
    {
        ...
        if (consumedString.startsWith(string, caseSensitive))
            result = DidMatch;
        prepend(SegmentedString2(consumedString));
        return result;
    }
    ...
    UChar m_pushedChar1;
    UChar m_pushedChar2;
    SegmentedSubstring2 m_currentString;
    UChar m_currentChar;
    int m_numberOfCharactersConsumedPriorToCurrentString;
    int m_numberOfCharactersConsumedPriorToCurrentLine;
    int m_currentLine;
    Deque<SegmentedSubstring2> m_substrings;
    bool m_closed;
    bool m_empty;
    unsigned char m_fastPathFlags;
    void (SegmentedString2::*m_advanceFunc)();
    void (SegmentedString2::*m_advanceAndUpdateLineNumberFunc)();
};
