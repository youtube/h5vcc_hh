...
bool base64Decode(const char* data, unsigned len, Vector<char>& out)
{
    ...
    out.grow(len);
/* FYWEBKITMOD BEGIN added code to ignore white spaces in encode (newer Webkit versions do this optionally & encoders seem to regularely put them into the stream these days -> hence...) */
	unsigned out_idx = 0;
    for (unsigned idx = 0; idx < len; idx++) {
        unsigned char ch = data[idx];
        if ((ch > 47 && ch < 58) || (ch > 64 && ch < 91) || (ch > 96 && ch < 123) || ch == '+' || ch == '/' || ch == '=')
            out[out_idx++] = base64DecMap[ch];
        else if (!isSpaceOrNewline(ch))
            return false;
    }
	len = out_idx;
/* FYWEBKITMOD END */
