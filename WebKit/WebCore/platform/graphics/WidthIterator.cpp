...
/* FYWEBKITMOD BEGIN */
		///////////////////////////////////////////////////////////////////////////////////
		/// As our rendering code does in cgx_platform_device.cpp, kerning between 
		/// characters must be considered when measuring the pixel width for a line of text
		/// in the layout code. Thus a few webkit sources had to be modified to create 
		/// the code path between webkit and the FY font library. (See WidthIterator.cpp, 
		/// SkPaint.h, and SkPaint.cpp)
		/// 
		/// The total width / advance of each character in a line of text is adjusted by 
		/// the total kerning between each character.
		/// 
		///////////////////////////////////////////////////////////////////////////////////
		if(currentCharacter < m_run.length())
		{
			const GlyphData& glyphDataLastC = m_font->glyphDataForCharacter(lastC, rtl);
			Glyph glyphLastC = glyphDataLastC.glyph;
			
			const GlyphData& glyphDataCurrC = m_font->glyphDataForCharacter(*cp, rtl);
			Glyph glyphCurrC = glyphDataCurrC.glyph;
			adjWidthKern += fontData->platformKerningPair(glyphLastC, glyphCurrC);
		}
/* FYWEBKITMOD END */
...
/* FYWEBKITMOD BEGIN */
    m_runWidthSoFar += widthSinceLastRounding + adjWidthKern;
    m_finalRoundingWidth = lastRoundingWidth + adjWidthKern;
/* FYWEBKITMOD END */
}
