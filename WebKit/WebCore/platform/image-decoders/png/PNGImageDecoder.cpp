...
void PNGImageDecoder::rowAvailable(unsigned char* rowBuffer, unsigned rowIndex, int interlacePass)
{
    ...
    if (destY < 0 || destY >= scaledSize().height())
        return;
#if !PLATFORM(FYMP) || PLATFORM(FYMP_VS) // FYWEBKITMOD excluded VisualStudio build
...
/* FYWEBKITMOD BEGIN */
	...
	// (pulled conditionals out of the inner loop etc. - assumes: Skia)
	//
	if (m_scaled)
	{
		if (hasAlpha)
		...
		else
		{
			for (int x = 0; x < width; ++x)
			{
				png_bytep pixel = row + m_scaledColumns[x] * 3;
				buffer.setRGBA(x, destY, pixel[0], pixel[1], pixel[2], 255);
			}
		}
	}
	else
	{
		...
		if (hasAlpha)
		{
			...
			for (int x = 0; x < width; ++x)
			{
				...
				unsigned alpha = pixel[3];
				RGBA32Buffer::PixelData *pDest = bm.getAddr32(x, destY);
				if (alpha == 0)
				{
					*pDest = 0;
					bAlpha = true;
					...
					*pDest = ((uint32_t)255 << 24 | pixel[0] << 16 | pixel[1] << 8 | pixel[2]);
				else
				{
                    float alphaPercent = alpha * (1.0f / 255.0f);
                    unsigned r = static_cast<unsigned>(pixel[0] * alphaPercent);
                    unsigned g = static_cast<unsigned>(pixel[1] * alphaPercent);
                    unsigned b = static_cast<unsigned>(pixel[2] * alphaPercent);
					*pDest = (alpha << 24 | r << 16 | g << 8 | b);
					bAlpha = true;
				}
			}
			if (bAlpha && !buffer.hasAlpha())
				buffer.setHasAlpha(true);
		}
		else
		{
			RGBA32Buffer::PixelData *pDest = bm.getAddr32(0, destY);
			png_bytep pixel = row;
			for (int x = 0; x < width; ++x)
			{
				*(pDest++) = ((uint32_t)255 << 24 | pixel[0] << 16 | pixel[1] << 8 | pixel[2]);
