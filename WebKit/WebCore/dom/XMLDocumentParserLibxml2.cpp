...
/* FYWEBKITMOD BEGIN included platforms using vsnprintf and introduced _FREE_TMPMEM instead of using same platform check again */
#undef _FREE_TMPMEM
#if COMPILER(MSVC) || COMPILER(RVCT) || COMPILER(MWERKS) || COMPILER(CLANG) || COMPILER(GHS) || (defined(FYMP_PLATFORM) && FYMP_PLATFORM == FYMP_PLATFORM_PS3) || (defined(FYMP_PLATFORM) && FYMP_PLATFORM == FYMP_PLATFORM_VITA)
    char m[1024];
