...
void Document::addMarker(Range *range, DocumentMarker::MarkerType type, String description)
{
    ...
    for (TextIterator markedText(range); !markedText.atEnd(); markedText.advance()) {
        ...
        int exception = 0;
        DocumentMarker marker = {type, (unsigned int)textPiece->startOffset(exception), (unsigned int)textPiece->endOffset(exception), description, false}; // FYWEBKITMOD casts to unsigned
