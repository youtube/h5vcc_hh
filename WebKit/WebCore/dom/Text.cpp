...
PassRefPtr<Text> Text::createWithLengthLimit(Document* document, const String& data, unsigned& charsLeft, unsigned maxChars)
{
    ...
    if (end < dataLength) {
        ...
        if (it && !isTextBreak(it, end - start)) // FYWEBKITMOD added check for iterator as our ICU package does not contain the data file needed to perform this check! (it really is just a safety precaution to not pass on parts of a text buffer with MBCS style encodings that are incomplete as we understand it)
