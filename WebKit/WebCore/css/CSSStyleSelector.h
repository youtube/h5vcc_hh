...
// FactorY Modification Begin: support for new code in header
#include "CSSSelector.h"
#include "Element.h"
// FactorY Modification End
...
// FactorY Modification: making sure the function (used only locally) is inlined
        ALWAYS_INLINE bool checkSelector(CSSSelector*);
...
// FactorY Modification Begin: pulled code partially into header
            ALWAYS_INLINE bool checkOneSelector(CSSSelector* sel, Element* e, HashSet<AtomicStringImpl*>* selectorAttrs, PseudoId& dynamicPseudo, bool isSubSelector, RenderStyle* elementStyle, RenderStyle* elementParentStyle) const
            {
                if (!e)
                    return false;
                ...
                // New "early out" check...
                if (!sel->hasAttribute() && sel->m_match != CSSSelector::PseudoClass && sel->m_match != CSSSelector::PseudoElement)
		            return true;

                return _checkOneSelector(sel, e, selectorAttrs, dynamicPseudo, isSubSelector, elementStyle, elementParentStyle);
            }
// FactorY Modification End
...
// FactorY Modification Begin: private support function for new in-header function
        private:
            bool _checkOneSelector(CSSSelector* sel, Element* e, HashSet<AtomicStringImpl*>* selectorAttrs, PseudoId& dynamicPseudo, bool isSubSelector, RenderStyle* elementStyle, RenderStyle* elementParentStyle) const;
// FactorY Modification End
        };
