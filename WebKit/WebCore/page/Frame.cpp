...
/* FYWEBKITMOD BEGIN */
#elif PLATFORM(FYMP)
    // We don't use the tiled backing store of webkit, but implement a similar system!
    // (we also expect to get dirty rectangles for -=outside=- the content area if we ask
    //  for them. Hence...)
    if (m_view)
        m_view->setPaintsEntireContents(true);
#endif
