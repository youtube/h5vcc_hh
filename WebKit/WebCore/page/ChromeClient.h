...
/* FYWEBKITMOD BEGIN */
#if PLATFORM(FYMP)
	virtual bool domWindow_fake_innerWidth(Frame *pFrame, int & width) const { return false; }
	virtual bool domWindow_fake_innerHeight(Frame *pFrame, int & height) const { return false; }

	enum PopUpState
	{
		ePopUpState_Create = 0,
		ePopUpState_Destroy,
		ePopUpState_Show,
		ePopUpState_Hide,
		ePopUpState_Update,
	...
	virtual void notify_PopUpState(PopUpState state, PopupMenuClient* pPopUpClient, const IntRect * pRect, FrameView* pFrameView, int index) { }

	virtual int getScreenIndex() const { return 0; }
#endif
/* FYWEBKITMOD END */
