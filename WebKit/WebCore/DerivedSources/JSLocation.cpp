...
static JSC_CONST_HASHTABLE HashTable JSLocationTable = { 19, 15, JSLocationTableValues, 0 };
...
#undef THUNK_GENERATOR
static JSC_CONST_HASHTABLE HashTable JSLocationPrototypeTable = { 9, 7, JSLocationPrototypeTableValues, 0 };
...
{
    return new (exec) JSLocationPrototype(globalObject, JSLocationPrototype::createStructure(globalObject->objectPrototype()));
}
...
EncodedJSValue JSC_HOST_CALL jsLocationPrototypeFunctionToString(ExecState* exec)
{
    ...
    if (!thisValue.inherits(&JSLocation::s_info))
        return throwVMTypeError(exec);
    JSLocation* castedThis = static_cast<JSLocation*>(asObject(thisValue));
    return JSValue::encode(castedThis->toString(exec));
}
