...
#include "JSArrayBuffer.h" // FYWEBKITMOD enabling binary response of XMLHttpRequest
...
JSC::JSValue jsXMLHttpRequestResponse(JSC::ExecState* exec, JSC::JSValue slotBase, const JSC::Identifier&)
{
	JSXMLHttpRequest* castedThis = static_cast<JSXMLHttpRequest*>(asObject(slotBase));
    UNUSED_PARAM(exec);
    XMLHttpRequest* imp = static_cast<XMLHttpRequest*>(castedThis->impl());

	if (imp->responseType() == "arraybuffer")
		{
        return toJS(exec, castedThis->globalObject(), WTF::getPtr(imp->responseArrayBuffer()));
		}

    return castedThis->responseText(exec);
}

JSC::JSValue jsXMLHttpRequestResponseType(JSC::ExecState* exec, JSC::JSValue slotBase, const JSC::Identifier&)
{
    JSXMLHttpRequest* castedThis = static_cast<JSXMLHttpRequest*>(asObject(slotBase));
    ExceptionCode ec = 0;
    XMLHttpRequest* imp = static_cast<XMLHttpRequest*>(castedThis->impl());
    JSC::JSValue result = jsString(exec, imp->responseType());
    return result;
}

void setJSXMLHttpRequestResponseType(JSC::ExecState* exec, JSC::JSObject* thisObject, JSC::JSValue value)
{
    JSXMLHttpRequest* castedThis = static_cast<JSXMLHttpRequest*>(thisObject);
    XMLHttpRequest* imp = static_cast<XMLHttpRequest*>(castedThis->impl());
    ExceptionCode ec = 0;
    imp->setResponseType(valueToStringWithNullCheck(exec, value), ec);
    setDOMException(exec, ec);
}
