...
{
    putDirect(exec->propertyNames().prototype, JSHTMLBlockquoteElementPrototype::self(exec, globalObject), DontDelete | ReadOnly);
}
...
{
    return new (exec) JSHTMLBlockquoteElementPrototype(globalObject, JSHTMLBlockquoteElementPrototype::createStructure(JSHTMLElementPrototype::self(exec, globalObject)));
}
...
JSValue jsHTMLBlockquoteElementCite(ExecState* exec, JSValue slotBase, const Identifier&)
{
    ...
    JSValue result = jsString(exec, imp->getAttribute(WebCore::HTMLNames::citeAttr));
    return result;
}
