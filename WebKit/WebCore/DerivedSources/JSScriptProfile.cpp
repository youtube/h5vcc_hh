...
const ClassInfo JSScriptProfilePrototype::s_info = { "ScriptProfilePrototype", 0, &JSScriptProfilePrototypeTable, 0 };
...
const ClassInfo JSScriptProfile::s_info = { "ScriptProfile", 0, &JSScriptProfileTable, 0 };
...
{
    return new (exec) JSScriptProfilePrototype(globalObject, JSScriptProfilePrototype::createStructure(globalObject->objectPrototype()));
}
...
JSValue jsScriptProfileUid(ExecState* exec, JSValue slotBase, const Identifier&)
{
    ...
    UNUSED_PARAM(exec);
    ScriptProfile* imp = static_cast<ScriptProfile*>(castedThis->impl());
    JSValue result = jsNumber(exec, imp->uid());
    return result;
}
