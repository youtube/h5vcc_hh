...
{
    putDirect(exec->propertyNames().prototype, JSProgressEventPrototype::self(exec, globalObject), DontDelete | ReadOnly);
}
...
{
    return new (exec) JSProgressEventPrototype(globalObject, JSProgressEventPrototype::createStructure(JSEventPrototype::self(exec, globalObject)));
}
...
EncodedJSValue JSC_HOST_CALL jsProgressEventPrototypeFunctionInitProgressEvent(ExecState* exec)
{
    ...
    unsigned loadedArg = exec->argument(4).toInt32(exec);
    unsigned totalArg = exec->argument(5).toInt32(exec);
