...
{
    putDirect(exec->propertyNames().prototype, JSHTMLPreElementPrototype::self(exec, globalObject), DontDelete | ReadOnly);
}
...
{
    return new (exec) JSHTMLPreElementPrototype(globalObject, JSHTMLPreElementPrototype::createStructure(JSHTMLElementPrototype::self(exec, globalObject)));
}
...
JSValue jsHTMLPreElementWidth(ExecState* exec, JSValue slotBase, const Identifier&)
{
    ...
    JSValue result = jsNumber(exec, imp->getIntegralAttribute(WebCore::HTMLNames::widthAttr));
    return result;
}
