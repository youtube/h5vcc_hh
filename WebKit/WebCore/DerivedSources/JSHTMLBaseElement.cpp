...
{
    putDirect(exec->propertyNames().prototype, JSHTMLBaseElementPrototype::self(exec, globalObject), DontDelete | ReadOnly);
}
...
{
    return new (exec) JSHTMLBaseElementPrototype(globalObject, JSHTMLBaseElementPrototype::createStructure(JSHTMLElementPrototype::self(exec, globalObject)));
}
...
JSValue jsHTMLBaseElementHref(ExecState* exec, JSValue slotBase, const Identifier&)
{
    ...
    JSValue result = jsString(exec, imp->getAttribute(WebCore::HTMLNames::hrefAttr));
    return result;
}
