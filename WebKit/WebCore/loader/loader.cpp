...
#endif
/* FYWEBKITMOD BEGIN added to integrate TextTrack requests into our loader (while integrating parts of r152754) */
...
Loader::Priority Loader::determinePriority(const CachedResource* resource) const
{
    ...
    switch (resource->type()) {
		...
		case CachedResource::ImageResource:
/* FYWEBKITMOD BEGIN added to integrate TextTrack requests into our loader (while integrating parts of r152754) */
#if ENABLE(VIDEO_TRACK) // FYMP - integrating parts of r152754
		case CachedResource::TextTrackResource:
#endif
/* FYWEBKITMOD END */
            return Low;
    ...
    for (unsigned p = 0; p <= High; p++)
    {
        ...
        for (RequestQueue::iterator it = requestsPending.begin(); it != end; ++it) {
            ...
            if (request == requestToCancel) {
                cache()->remove(request->cachedResource());
                requestToCancel->docLoader()->decrementRequestCount(request->cachedResource());
