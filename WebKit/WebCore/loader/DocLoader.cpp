...
// FYWEBKITMOD BEGIN - modification due to feature 'SubCache'. See comment to class SubCache for more details.
...
CachedTextTrack* DocLoader::requestTextTrack(const String& url)
{
	return static_cast<CachedTextTrack*>(requestResource(CachedResource::TextTrackResource, url, String()));
}
#endif
/* FYWEBKITMOD END */
...
/* FYWEBKITMOD BEGIN adding TextTrackResource to DocLoader (while integrating parts of r152754) */
#if ENABLE(VIDEO_TRACK)
...
// FYMP END
    ...
    if (resource) {
        ...
        m_documentResources.add(resource); // FYWEBKITMOD modification due to feature 'SubCache'. See comment to class SubCache for more details.
...
/* FYWEBKITMOD BEGIN added 2011/04/20 */
