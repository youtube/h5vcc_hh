...
#include "CachedTextTrack.h" // FYMP - integrating parts of r152754
...
#endif
/* FYWEBKITMOD BEGIN integrating TextTrackResource into our WebKit's Loader implementation. (while integrating parts of r152754) */
#if ENABLE(VIDEO_TRACK)
    case CachedResource::TextTrackResource:
        return new CachedTextTrack(url.string());
#endif
/* FYWEBKITMOD END */
    default:
        break;
...
CachedResource* Cache::requestResource(DocLoader* docLoader, CachedResource::Type type, const KURL& url, const String& charset, bool requestIsPreload)
{
    ...
    if (!resource) {
        ...
        if (!disabled())
            ...
            resource->setInCache(false);
/* FYWEBKITMOD BEGIN removed the adding of the DocLoader pointer. It would end up with a set DocLoader pointer, BUT not be in the Cache & m_documentResources -> DocLoader can be accessed after it dies)
...
// FYWEBKITMOD BEGIN integrated feature 'SubCache'. See comment to class SubCache for more details.
...
/* FYWEBKITMOD BEGIN */
#if defined(FYMP_PLATFORM) && FYMP_BUILDTYPE != FYMP_BUILDTYPE_FINAL
#include "std_DebugIO.h"
void dumpWebCoreCache(void)
	{
	WebCore::Cache::Statistics s = WebCore::cache()->getStatistics();
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%11s %11s %11s %11s %11s %11s %11s", "", "Count", "Size", "LiveSize", "DecodedSize", "PurgeableSize", "PurgedSize");
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %-11s %-11s %-11s %-11s %-11s %-11s", "-----------", "-----------", "-----------", "-----------", "-----------", "-----------", "-----------");
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %11d %11d %11d %11d %11d %11d", "Images", s.images.count, s.images.size, s.images.liveSize, s.images.decodedSize, s.images.purgeableSize, s.images.purgedSize);
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %11d %11d %11d %11d %11d %11d", "CSS", s.cssStyleSheets.count, s.cssStyleSheets.size, s.cssStyleSheets.liveSize, s.cssStyleSheets.decodedSize, s.cssStyleSheets.purgeableSize, s.cssStyleSheets.purgedSize);
#if ENABLE(XSLT)
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %11d %11d %11d %11d %11d %11d", "XSL", s.xslStyleSheets.count, s.xslStyleSheets.size, s.xslStyleSheets.liveSize, s.xslStyleSheets.decodedSize, s.xslStyleSheets.purgeableSize, s.xslStyleSheets.purgedSize);
#endif
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %11d %11d %11d %11d %11d %11d", "JavaScript", s.scripts.count, s.scripts.size, s.scripts.liveSize, s.scripts.decodedSize, s.scripts.purgeableSize, s.scripts.purgedSize);
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %11d %11d %11d %11d %11d %11d", "Fonts", s.fonts.count, s.fonts.size, s.fonts.liveSize, s.fonts.decodedSize, s.fonts.purgeableSize, s.fonts.purgedSize);
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "%-11s %-11s %-11s %-11s %-11s %-11s %-11s", "-----------", "-----------", "-----------", "-----------", "-----------", "-----------", "-----------");
    STDdbg::Printf(STDdbg::IO_CLASS_DEFAULT, "Live/Dead: %u / %u  = %u\n", s.liveSize, s.deadSize, s.liveSize+s.deadSize);
	}
#endif
/* FYWEBKITMOD END */
