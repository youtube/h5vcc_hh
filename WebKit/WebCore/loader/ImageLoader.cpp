...
/* FYWEBKITMOD BEGIN added code to not set the DocLoader & add the newImage instance if the Cache() is disabled (it would end up with a set DocLoader pointer, BUT not be in the Cache & m_documentResources -> DocLoader can be accessed after it dies) */
			if (!cache()->disabled()) {
/* FYWEBKITMOD END */
				newImage->setDocLoader(document->docLoader());
				document->docLoader()->m_documentResources.add(newImage); // FYWEBKITMOD - modified due to feature 'SubCache'. See comment to class SubCache for more details.
			}
