...
//	The GHS compiler seems to not remove these reference as it has a prototype in the class?!? -> should be unused by design!
void* ClipRects::operator new(size_t sz) throw()
{
...
#if FYMP_PLATFORM == FYMP_PLATFORM_WIIU
//	The GHS compiler seems to not remove these reference as it has a prototype in the class?!? -> should be unused by design!
void* RenderLayer::operator new(size_t sz) throw()
{
	ASSERT(!"*** Not implemented!");
    return NULL;
}
...
void RenderLayer::styleChanged(StyleDifference diff, const RenderStyle*)
{
    ...
    if (m_backing && diff >= StyleDifferenceRepaint
		 && diff != StyleDifferenceLayoutPositionedMovementOnly) // FYWEBKITMOD
