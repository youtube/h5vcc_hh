...
IntRect RenderSlider::thumbRect()
{
    ...
    if (style()->appearance() == SliderVerticalPart || style()->appearance() == MediaVolumeSliderPart) {
        ...
        thumbRect.setY(contentRect.y() + static_cast<int>(nextafter(float((contentRect.height() - thumbRect.height()) + 1), 0.0f) * (1 - fraction))); // FYWEBKITMOD cast to float
    } else {
        thumbRect.setX(contentRect.x() + static_cast<int>(nextafter(float((contentRect.width() - thumbRect.width()) + 1), 0.0f) * fraction)); // FYWEBKITMOD cast to float
