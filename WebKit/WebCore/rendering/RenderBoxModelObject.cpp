...
#if PLATFORM(FYMP)
        /*
            On the FYMP platform we assume that we can render the root's background color infinitely far
            outside the content rectangle. Hence we simply use the paintInfo's rectangle if this is the
            root. Otherwise we follow what all other platforms do.
        */
        IntRect rect;
		// Root in this view & the view is at the root of the page?
        if (isRoot && !view()->frameView()->parent())
            rect = paintInfo.rect;	// Yes. Let the background fill extend "forever"...
        else
        {
            rect = IntRect(tx, ty, w, h);
